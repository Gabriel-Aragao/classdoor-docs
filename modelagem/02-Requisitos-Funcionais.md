...
| :--- | :--- | :--- | :---: | :---: |
| **RF01** | Autenticação | O sistema deve permitir o cadastro de novos usuários com nome, e-mail válido, senha segura e perfil (`Estudante` ou `Professor`). | **MUST** | US01 |
| **RF02** | Autenticação | O sistema deve autenticar usuários existentes via e-mail e senha, gerando token JWT de sessão. | **MUST** | US02 |
| **RF03** | Autenticação | O sistema deve permitir a solicitação de recuperação de senha mediante envio de link/código por e-mail. | **MUST** | US02 |
...
| **RF10** | Motor de Avaliação | O sistema deve permitir que estudantes autenticados enviem avaliações anônimas contendo nota geral (1-5), dificuldade (1-5), recomendação (Sim/Não) e comentário textual (sem tags). | **MUST** | US05 |
...
| **RF14** | Gestão Docente | O sistema deve permitir que professores criem turmas, adicionem alunos em lote (Bulk Import) e configurem o toggle de liberação de avaliações. | **MUST** | US08 |
| **RF15** | Analytics & Gestão | O sistema deve fornecer um dashboard analítico para professores com scorecards, histograma de notas e gráficos de evolução semestral de médias. | **SHOULD** | US09 |
