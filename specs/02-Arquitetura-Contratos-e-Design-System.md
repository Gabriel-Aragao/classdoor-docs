...
| **2** | `2. Home & Busca Global (US03)` | **US03** | `/`<br>`1440 × 960 px` | • **Navbar Autenticada:** Logo, identificação do usuário com perfil ("Lucas Silva - Estudante") e botão "Sair" (`bi-box-arrow-right`).<br>• **Hero Section:** Título de impacto e caixa de busca global com `bi-search` e debounce de 300ms.<br>• **Sidebar de Filtros Acadêmicos:** Filtro por tipo (Todos, Docentes, Cursos), Departamento (dropdown), Semestre Letivo (`2026.1`, `2026.2`), slider de nota mínima (1.0★ a 5.0★) e botão "Limpar Filtros".<br>• **Grid de Resultados & Destaques (Sem Tags de Destaque):** Cards minimalistas estruturados com avatar/iniciais, nome do docente/disciplina, departamento, nota média com `bi-star-fill` (#F39C12), volume de reviews e botão alinhado na base "Ver Perfil Completo". |
| **3** | `3. Perfil de Docente & Reviews (US04 & US07)` | **US04**<br>**US07** | `/professores/:id`<br>`1440 × 1000 px` | • **Header do Perfil:** Botão "← Voltar à Busca", avatar com iniciais, nome do professor/disciplina, departamento, badges de disciplinas ministradas e botão de destaque "⭐ Avaliar Professor" (`#2C3E50`).<br>• **Painel de Scorecards (Métricas):** 4 cards com Nota Média Geral (ex: `4.6 ★`), Taxa de Recomendação (`88%`), Nível de Dificuldade (`3.2 / 5.0`) e Total de Reviews (`42 feedbacks`).<br>• **Coluna Lateral:** Card com Histograma de Distribuição de Notas (1 a 5 estrelas em barras horizontais coloridas).<br>• **Feed de Avaliações (Sem Tags Pedagógicas):** Seletor de ordenação (*Mais Recentes*, *Melhor Avaliadas*, *Mais Úteis*); cards de avaliação contendo badge de anonimato (`bi-shield-check` - "Estudante Anônimo") ou nome público (se nominal autorizado), data, semestre da turma, nota geral, parecer qualitativo completo, indicador de recomendação e botão de interação "Útil 👍" (`bi-hand-thumbs-up-fill` - US07) com contador e idempotência. |
